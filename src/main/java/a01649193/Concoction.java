...
public class Concoction extends Potion {
	...
	public Concoction(String name,	int usages, int price, int weight, int health, int mana, List<Spell> spells) {
		...
		if(spells==null){
			throw new IllegalArgumentException("spells==null");
		}
		if (health == 0 && mana == 0 && spells.isEmpty()) {
			throw new IllegalArgumentException("The potion must have at least one effect");
		}
	...
	public String additionalOutputString() {
		StringBuilder result = new StringBuilder(super.additionalOutputString());
		...
		if (!spells.isEmpty()) {
			result.append("; cast ").append(spells);
		}
