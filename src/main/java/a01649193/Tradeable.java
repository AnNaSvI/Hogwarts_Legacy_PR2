...
public interface Tradeable {
	...
	default boolean give(Trader giver, Trader taker) {
		if(giver ==null || taker==null || giver.equals(taker)){
			throw new IllegalArgumentException("giver or taker is null or they are the same object");
		}
	...
	default boolean purchase(Trader seller, Trader buyer) {
		if(seller ==null || buyer==null || seller.equals(buyer)){
			throw new IllegalArgumentException("seller or buyer is null or they are the same object,");
		}
		if(seller.possesses(this) && buyer.canAfford(this.getPrice()) && buyer.hasCapacity(getWeight())){
